...
- 平动同步带：HTD 5M，齿距 5 mm
- 平动动力轮：80 齿（单圈线位移 = 80 × 5 mm = 400 mm）
- 旋转传动比：主动齿:被动齿 = 50:130（等效 5:13）

#### 代码单位说明

`Robo_Chassis.c` 里的 `expect_driver_speed` 与 `Get_Motor_Speed_Data()` 均使用电机角速度，单位为 `rad/s`。

#### 换算公式

- 平动线速度：`v = ω × (0.4 / 2π)`（m/s，ω 为电机角速度 rad/s）
- 旋转输出角速度：`ω_out = ω_motor × (5 / 13)`（rad/s）

#### 当前可用速度结论

- 平动测试值 `ω = 500 rad/s`：`v ≈ 31.83 m/s`（机构理论值）
- 旋转测试值 `ω = 1000 rad/s`：`ω_out ≈ 384.62 rad/s`
- 慢动作实测（10 s 慢动作内转 7 圈，按 240fps→30fps 默认 8 倍慢放）：
	- 真实时间 `t = 10 / 8 = 1.25 s`
	- 实际角速度 `ω = 7 × 2π / 1.25 ≈ 35.2 rad/s`（约 336 rpm）


### 3508性能参考
3508电机大疆官方[网址](https://www.robomaster.com/zh-CN/products/components/general/M3508)

电机参数截图见：[3508电机数据](./03-Docs/Images/3508电机数据.png)
...
- [移动部分总装.step](./01-Model/移动部分总装.step)
