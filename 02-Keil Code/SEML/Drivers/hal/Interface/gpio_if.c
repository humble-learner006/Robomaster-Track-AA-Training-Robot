...
void SEML_GPIO_Pin_Set(GPIO_Handle_t *GPIO_Handle)
{
	if (GPIO_Handle->GPIO_Part == NULL)
		return;
	HAL_GPIO_WritePin((GPIO_Handle)->GPIO_Part, (GPIO_Handle)->GPIO_Pin, GPIO_PIN_SET);
}
/**
 * @brief GPIO引脚输出置0
 * @param GPIO_Handle GPIO句柄
...
void SEML_GPIO_Pin_Reset(GPIO_Handle_t *GPIO_Handle)
{
	if (GPIO_Handle->GPIO_Part == NULL)
		return;
	HAL_GPIO_WritePin((GPIO_Handle)->GPIO_Part, (GPIO_Handle)->GPIO_Pin, GPIO_PIN_RESET);
}
/**
 * @brief GPIO引脚输出
 * @param GPIO_Handle GPIO句柄
...
void SEML_GPIO_Pin_Output(GPIO_Handle_t *GPIO_Handle, uint16_t status)
{
	if (GPIO_Handle->GPIO_Part == NULL)
		return;
	HAL_GPIO_WritePin((GPIO_Handle)->GPIO_Part, (GPIO_Handle)->GPIO_Pin, (GPIO_PinState)status);
}
/**
 * @brief GPIO引脚输出反转
 * @param GPIO_Handle GPIO句柄
